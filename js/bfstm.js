/**
 * BFSTM decoder (plain JS, no TypeScript).
 * Based on https://github.com/kenrick95/nikku (MIT)
 * Supports PCM8, PCM16, Nintendo DSP-ADPCM (big & little endian).
 */
...
export class Bfstm {
  ...
  #getTrackDescriptions(numberChannels) {
  ...
  #getDefaultTrackDescriptions(numberChannels) {
    const tracks = [];
  ...
  #decodeDspAdpcm(data, output, coefficients, history) {
    let dataOffset = 0;
  ...
  #resolveReference(offset, base, name) {
  ...
  #resolveOptionalReference(offset, base, name) {
  ...
  #assertRange(offset, length, name) {
    if (
